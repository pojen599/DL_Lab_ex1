...
def run_episode(env, agent, rendering=True, max_timesteps=1000):
    ...
    while True:
        ...
        action_idx = torch.argmax(action_idx) # Get the index of the action with the highest probability
        a = utils.id_to_action(action_idx.item()) # Use .item() to get the integer value from the tensor
...
if __name__ == "__main__":
    ...
    fname = os.path.join(results_dir, "results_bc_agent-%s.json" % datetime.now().strftime("%Y%m%d-%H%M%S"))
